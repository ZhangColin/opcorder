...
function SubmissionForm({
  ...
  function doSubmit() {
    ...
    setConfirming(false);
    onSubmit({ content, attachments, urls: cleanUrls });
  }
        ...
        <div className="flex flex-col gap-1.5">
          {attachments.map((a, i) => (
            <div key={i} className="flex items-center gap-2 text-xs text-slate-700 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2">
              ...
              <button type="button" onClick={() => setAttachments(p => p.filter((_, j) => j !== i))} className="text-slate-300 hover:text-red-500 shrink-0"><X size={11} /></button>
            </div>
          ))}
          <button
            type="button"
        ...
        <div className="flex flex-col gap-2">
          {urls.map((u, i) => (
            <div key={i} className="flex items-center gap-2">
              ...
              {urls.length > 1 && (
                <button type="button" onClick={() => setUrls(p => p.filter((_, j) => j !== i))} className="text-slate-300 hover:text-red-500 p-1"><X size={14} /></button>
              )}
            </div>
          ))}
          {urls.length < 5 && (
            <button type="button" onClick={() => setUrls(p => [...p, ""])} className="text-xs text-primary hover:underline text-left">+ 添加链接</button>
          )}
        </div>
      </div>
      ...
      {confirming && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full mx-4 p-6 flex flex-col gap-4">
            ...
            <p className="text-sm text-slate-500">提交后不可修改，请确认您的答案已完整填写。是否确认提交？</p>
...
/* ─── Countdown block ─── */
function CountdownBlock({ deadline, label, expired }: { deadline: string; label: string; expired: boolean }) {
  const cd = useCountdown(deadline);
  if (expired || cd.expired) {
    return <div className="flex items-center gap-1.5 text-xs text-red-500 font-semibold"><AlertCircle size={13} />提交已截止</div>;
  }
  return (
    <div className="flex items-center gap-2 text-sm">
      <Clock size={14} className={cd.d === 0 && Number(cd.h) < 2 ? "text-red-500" : "text-amber-500"} />
      <span className="text-slate-500 text-xs">{label}剩余：</span>
      <span className={`font-black tabular-nums ${cd.d === 0 && Number(cd.h) < 2 ? "text-red-600" : "text-amber-600"}`}>
...
function SectionCard({ title, children }: { title: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm">
      <div className="px-6 py-4 border-b border-slate-50 flex items-center gap-2">{title}</div>
...
/* ─── Submitted view ─── */
function SubmittedView({ content, attachments, urls }: { content: string | null; attachments: Array<{ name: string; url: string }> | null; urls: string[] | null }) {
  return (
    <div className="flex flex-col gap-4">
...
/* ─── Main ─── */
export default function OpcV2ContestDetail() {
  ...
  const testMut = useMutation({
    ...
    onSuccess: () => { toast({ title: "测试题提交成功！" }); qc.invalidateQueries({ queryKey: ["opc-contest-reg", registrationId] }); qc.invalidateQueries({ queryKey: ["opc-my-contests"] }); },
  ...
  const assignMut = useMutation({
    ...
    onSuccess: () => { toast({ title: "测试单提交成功！" }); qc.invalidateQueries({ queryKey: ["opc-contest-reg", registrationId] }); qc.invalidateQueries({ queryKey: ["opc-my-contests"] }); },
    onError: (e: Error) => toast({ title: "提交失败", description: e.message, variant: "destructive" }),
  });
      ...
      <div className="py-6 flex flex-col gap-6">
        ...
        <div>
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            ...
            <Trophy size={28} className="text-amber-400 mt-1" />
          </div>
        </div>
        ...
        <SectionCard
          title={
            <div className="flex items-center justify-between w-full flex-wrap gap-2">
              ...
              {!reg.testSubmittedAt && testDeadline && (
                <CountdownBlock deadline={testDeadline} label="提交" expired={testExpired} />
              )}
              {reg.testSubmittedAt && (
                <span className="text-xs text-emerald-600 font-semibold flex items-center gap-1"><CheckCircle2 size={13} />已提交</span>
              )}
            </div>
          }
        >
          {reg.testQuestion ? (
            <div className="mb-5">
              <h3 className="text-base font-bold text-slate-800 mb-3">{reg.testQuestion.title}</h3>
              {reg.testQuestion.content && (
                <div className="bg-slate-50 rounded-xl p-4 mb-3">
                  <MarkdownContent content={reg.testQuestion.content} />
                </div>
              )}
              {reg.testQuestion.attachments && reg.testQuestion.attachments.length > 0 && (
                <div className="mb-3">
                  <p className="text-xs font-semibold text-slate-400 mb-1.5 flex items-center gap-1"><Paperclip size={11} />题目附件</p>
                  <AttachmentList items={reg.testQuestion.attachments} />
                </div>
              )}
            </div>
          ) : (
          ...
          <div className="border-t border-slate-50 pt-4">
            ...
            {reg.testSubmittedAt ? (
              <SubmittedView
                content={reg.testContent}
                attachments={reg.testAttachments}
                urls={reg.testUrls}
              />
            ) : testExpired ? (
              ...
              <SubmissionForm
                onSubmit={data => testMut.mutate(data)}
        ...
        {showAssignment && (
          <SectionCard
            title={
              <div className="flex items-center justify-between w-full flex-wrap gap-2">
                ...
                {!reg.assignmentSubmittedAt && assignDeadline && (
                  <CountdownBlock deadline={assignDeadline} label="提交" expired={assignExpired} />
                )}
          ...
          >
            {reg.assignmentQuestion ? (
              <div className="mb-5">
                <h3 className="text-base font-bold text-slate-800 mb-3">{reg.assignmentQuestion.title}</h3>
                {reg.assignmentQuestion.content && (
                  <div className="bg-slate-50 rounded-xl p-4 mb-3">
                    <MarkdownContent content={reg.assignmentQuestion.content} />
                  </div>
                )}
                {reg.assignmentQuestion.attachments && reg.assignmentQuestion.attachments.length > 0 && (
                  <div className="mb-3">
                    <p className="text-xs font-semibold text-slate-400 mb-1.5 flex items-center gap-1"><Paperclip size={11} />题目附件</p>
                    <AttachmentList items={reg.assignmentQuestion.attachments} />
                  </div>
                )}
              </div>
            ) : (
            ...
            <div className="border-t border-slate-50 pt-4">
              ...
              {reg.assignmentSubmittedAt ? (
                <SubmittedView
                  content={reg.assignmentContent}
                  attachments={reg.assignmentAttachments}
                  urls={reg.assignmentUrls}
                />
              ) : !benefitUnlocked ? (
                <p className="text-sm text-slate-400">权益发放时间到达后可提交</p>
              ) : assignExpired ? (
                <div className="flex items-center gap-2 text-sm text-red-500 font-semibold">
                  <AlertCircle size={15} /> 提交时间已截止
                </div>
                ...
                <SubmissionForm
                  onSubmit={data => assignMut.mutate(data)}
        ...
        {(reg.testGrade || reg.assignmentGrade) && (
          <SectionCard
            title={<span className="font-bold text-blue-900 text-base flex items-center gap-2"><Trophy size={16} className="text-amber-400" />评级结果</span>}
          >
