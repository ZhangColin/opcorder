import {
  Wallet, Lock, Clock, CreditCard, TrendingUp, AlertCircle,
  Download, ShieldCheck, BarChart3, Gavel, Zap,
  ArrowDownLeft, Landmark, Percent, MoreHorizontal,
} from "lucide-react";
...
const STAT_CARDS = [
  ...
  },
  {
    ...
    value: "¥3,450,200",
    badge: "托管中",
    badgeCls: "text-muted-foreground",
    BadgeIcon: null,
    icon: Lock,
  ...
  },
  {
    ...
    value: "¥892,400",
    badge: "14件到期",
    badgeCls: "text-destructive",
    BadgeIcon: Clock,
    icon: Clock,
    iconCls: "bg-emerald-900/10 text-emerald-900",
    sub: "等待里程碑4",
  },
  {
    label: "平台手续费",
    value: "¥421,000",
    badge: "本月累计",
    badgeCls: "text-secondary font-bold",
    BadgeIcon: null,
    icon: CreditCard,
    iconCls: "bg-blue-900/10 text-blue-900",
    bars: true,
  },
];
...
const TRANSACTIONS = [
  {
    ...
    title: "里程碑支付: OPC-9921",
    meta: "2024年10月24日 • 14:22 • 批次ID: 772183",
    amount: "+¥124,000.00",
    status: "已完成",
  ...
  },
  {
    ...
    title: "托管存款: 项目「天剑」",
    meta: "2024年10月23日 • 09:15 • 交易ID: 110928",
    amount: "-¥500,000.00",
    status: "处理中",
  ...
  {
    icon: Percent,
    iconCls: "bg-slate-200 text-slate-500",
    borderCls: "border-slate-400",
    title: "平台服务费: 10月第3周",
    meta: "2024年10月21日 • 18:00 • 参考号: FEE-882",
    amount: "¥12,400.00",
    status: "已对账",
    statusCls: "text-muted-foreground",
    positive: true,
  },
];
        ...
        {STAT_CARDS.map(s => {
          ...
          return (
            <div key={s.label} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
              ...
              {s.sub && (
                <p className="text-[10px] text-muted-foreground mt-2 uppercase tracking-widest">{s.sub}</p>
              )}
              {s.bars && (
                  ...
                  {SETTLEMENTS.map(row => (
                    <tr key={row.id} className="hover:bg-slate-50/60 transition-colors">
                      <td className="px-6 py-5">
                        <p className="font-bold text-foreground">{row.project}</p>
          ...
          <section>
            <div className="flex items-center justify-between mb-5">
              ...
              <div className="flex gap-2">
                <select className="text-xs bg-slate-100 border-none rounded-lg py-1.5 px-3 focus:ring-primary outline-none">
                  <option>全部日期</option>
                  <option>最近7天</option>
                  <option>最近30天</option>
                </select>
                ...
                return (
                  <div key={i} className={`flex items-center justify-between p-4 bg-white rounded-2xl hover:bg-slate-50 transition-all border-l-4 shadow-sm ${tx.borderCls}`}>
                    ...
                    <div className="text-right shrink-0">
                      <p className="font-extrabold text-foreground">{tx.amount}</p>
            ...
            <div className="space-y-5">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">资金存放于一类国有银行账户。</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">增值税/个税自动化处理，覆盖全部支付。</p>
                </div>
              </div>
              <div className="p-4 bg-slate-50 rounded-xl text-center">
                ...
                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide">验证账本完整性</p>
              </div>
            </div>
          </div>
