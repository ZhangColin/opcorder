...
import {
  ...
  Mail, Lock, User, Building2, CheckCircle2, AlertCircle, Phone, ShieldCheck,
} from "lucide-react";
    ...
    if (tab === "register") {
      ...
      try {
        const res = await fetch(`${API_BASE}/api/auth/register`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ nickname: name.trim(), email: identifier.trim().toLowerCase(), phone: phone.trim(), smsCode: smsCode.trim(), password, role }),
        });
        const data = await res.json();
    ...
    <div className="min-h-screen flex flex-col bg-[#f9f9fc] text-[#1a1c1e]">
      ...
      <footer className="w-full py-6 px-8 bg-slate-50 border-t border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <nav className="flex gap-6 flex-wrap justify-center">
          ...
          <a href="/support" className="text-[10px] text-slate-400 font-medium hover:text-primary transition-colors tracking-wide">机构支持</a>
