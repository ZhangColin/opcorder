...
/* NOTE: Upload, IdCard kept for icons still used inline in this file */
import { DocCardUpload, type DocType } from "@/components/DocCardUpload";
import {
  useGetCurrentUser,
  useGetOpcProfile,
  useUpdateOpcProfile,
...
export default function AccountSettings() {
  ...
  const profileInitialRef     = useRef<ProfileForm>(EMPTY_PROFILE);
  const settlementInitialRef  = useRef<SettlementForm>(EMPTY_SETTLEMENT);
  ...
  /* Profile field helpers — compare against initial snapshot to determine dirty */
  const setP = useCallback(<K extends keyof ProfileForm>(k: K, v: ProfileForm[K]) => {
    setProfileForm(prev => {
      const next = { ...prev, [k]: v };
      setProfileDirty(JSON.stringify(next) !== JSON.stringify(profileInitialRef.current));
      return next;
    });
  }, []);
