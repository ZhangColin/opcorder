...
function GradeForm({ label, onSubmit, loading, disabled }: {
  ...
  if (disabled) return <div className="text-sm text-slate-400 mt-3">（已评级，只读）</div>;
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm p-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-base font-bold text-blue-900">{label}</h3>
      ...
      {!content && !attachments?.length && !urls?.length ? (
        <p className="text-sm text-slate-400 py-4 text-center">尚未提交</p>
      ) : (
...
export default function ContestRegistrationAdminDetail({ inlineId }: { inlineId: number }) {
  ...
  async function gradeAssignment(grade: "A" | "B" | "C" | "fail", note: string) {
    ...
    try {
      await adminPost(`/api/admin/contests/registrations/${inlineId}/grade-assignment`, { grade, note });
