...
import { Plus, Edit2, Trash2, Loader2, Search, X, ChevronDown } from "lucide-react";
...
export default function ContestQuestions() {
  ...
  const { data, isLoading } = useQuery<{ items: ContestQuestion[]; total: number; page: number; pageSize: number }>({
    queryKey: ["admin-contest-questions", catFilter, page],
    queryFn: () => adminGet(`/api/admin/contests/questions?${params}`),
  });

  const saveMut = useMutation({
    mutationFn: async (payload: { catCategoryId: number; title: string; content: string }) => {
      if (editTarget) {
  ...
  function handleSubmit() {
    ...
    saveMut.mutate({ catCategoryId: Number(form.catCategoryId), title: form.title, content: form.content });
  }
      ...
      <div className="bg-white rounded-2xl overflow-hidden shadow-sm">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-50">
            {isLoading ? (
              <tr><td colSpan={4} className="px-6 py-10 text-center text-slate-400"><div className="flex items-center justify-center gap-2"><Loader2 size={18} className="animate-spin" /><span className="text-sm">加载中…</span></div></td></tr>
            ) : !data?.items?.length ? (
              <tr><td colSpan={4} className="px-6 py-10 text-center text-sm text-slate-400">暂无题目</td></tr>
            ) : data.items.map(q => (
