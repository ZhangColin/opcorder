...
const STATUS_CONFIG: Record<string, { label: string; color: string }> = {
  ...
  esign_pending:            { label: "待您签署",   color: "bg-violet-100 text-violet-700" },
      ...
      <div className="mt-6 space-y-5">
        ...
        {contract.status === "esign_pending" && (
          <div className="bg-violet-50 border border-violet-200 rounded-2xl p-5">
            <p className="text-sm font-bold text-violet-800 mb-1">请前往 e签宝完成电子签署</p>
            <p className="text-xs text-violet-600 mb-4">运营方已发起电子签署，平台已盖章，请点击下方链接完成您的签名。</p>
            {contract.esignSignUrl ? (
              <a href={contract.esignSignUrl} target="_blank" rel="noreferrer"
                className="inline-flex items-center gap-2 bg-violet-600 text-white rounded-xl px-5 py-2.5 text-sm font-bold hover:bg-violet-700 transition-colors">
                <ExternalLink size={14} /> 前往 e签宝签署
              </a>
            ) : (
              <p className="text-xs text-violet-500 italic">签署链接正在生成中，请稍后刷新页面</p>
            )}
          </div>
        )}
        ...
        {(contract.esignSignedFileUrl || contract.signedFileUrl) && (
          <div className="bg-green-50 border border-green-200 rounded-2xl p-5">
            <h3 className="text-sm font-bold text-green-800 mb-3">🎉 已完成签署 — 下载盖章合同</h3>
            <a href={contract.esignSignedFileUrl ?? contract.signedFileUrl!} target="_blank" rel="noreferrer"
              className="inline-flex items-center gap-2 bg-green-600 text-white rounded-xl px-5 py-2.5 text-sm font-bold hover:bg-green-700 transition-colors">
              <ExternalLink size={14} /> 查看 / 下载盖章合同
            </a>
          </div>
        )}
