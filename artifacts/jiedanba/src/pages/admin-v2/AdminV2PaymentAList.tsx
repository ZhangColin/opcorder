...
import {
  Loader2, ChevronRight, AlertTriangle, CreditCard,
  CheckCircle2, Clock,
} from "lucide-react";
...
export default function AdminV2PaymentAList() {
  ...
  const pendingTotal = items.filter(p => p.status === "pending" || p.status === "awaiting_review").reduce((s, p) => s + p.amount, 0);
  const counts = STATUS_TABS.reduce((acc, tab) => {
    if (tab.value === "") acc[tab.value] = items.length;
    else if (tab.value === "overdue") acc[tab.value] = overdueItems.length;
    else acc[tab.value] = items.filter(p => p.status === tab.value).length;
    return acc;
        ...
        {overdueItems.length > 0 && !statusFilter && (
          <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl p-4">
            ...
            <p className="text-sm text-red-800 font-medium">
              <strong>{overdueItems.length}</strong> 项收款已逾期，请尽快跟进
            </p>
          </div>
        )}
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {pendingTotal > 0 && (
            <p className="ml-auto text-xs text-slate-500">
              待收合计 <span className="font-black text-slate-700">{fmtAmt(pendingTotal)}</span>
            </p>
          )}
        </div>
              ...
              return (
                <div key={item.id} onClick={go}
                  className={`bg-white rounded-2xl border shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all cursor-pointer group px-5 py-4 ${
                    overdue ? "border-red-200 bg-red-50/30" : soon ? "border-orange-100 bg-orange-50/20" : "border-slate-100"
                  }`}>
                  <div className="flex items-start justify-between gap-4 mb-2.5">
                    <p className="text-[15px] font-bold text-slate-800 group-hover:text-primary transition-colors leading-snug flex-1 min-w-0 truncate">
                      {item.demandTitle ?? item.title ?? "（无关联需求）"}
                    </p>
                  ...
                  <div className="flex items-end gap-6 text-xs flex-wrap">
                    <div>
                      ...
                      <p className="text-slate-600">
                        第 {item.itemNo ?? 1} 期{item.description ? `  · ${item.description}` : item.title ? `  · ${item.title}` : ""}
                      </p>
                    </div>
                    {item.dueDate && (
                      <div>
                        <p className="text-[10px] text-slate-400 uppercase tracking-wider mb-0.5">应收日期</p>
                    ...
                    <div className="ml-auto">
                      <p className="text-[10px] text-slate-400 uppercase tracking-wider mb-0.5">收款金额</p>
