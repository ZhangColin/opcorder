...
import { Loader2, PackageCheck, Clock, CheckCircle2, XCircle, Link2, X } from "lucide-react";
...
export default function AdminV2DeliveryADetail({ inlineId }: { inlineId?: number } = {}) {
  ...
  const act = async (endpoint: string, body?: Record<string, string>, successMsg?: string) => {
    setActing(true);
    try {
      await v2Post(`/deliverables-a/${id}/${endpoint}`, body);
            ...
            <div className="mt-4 flex gap-2 justify-end">
              {isPending && (
                <>
                  ...
                  <button onClick={() => act("publisher-confirm", undefined, "已确认交付")} disabled={acting}
              ...
              {isRejected && (
                <button onClick={() => act("resubmit", undefined, "已重新提交")} disabled={acting}
                  className="px-4 py-2 text-sm bg-primary text-white rounded-xl font-bold hover:bg-primary/90 disabled:opacity-50 transition-colors">
                  {acting ? "提交中…" : "重新提交"}
                </button>
              )}
            </div>
      ...
      {showReject && (
        <RejectModal
          ...
          onConfirm={async (reason) => {
            await act("publisher-reject", { reason }, "已驳回交付");
            setShowReject(false);
          }}
        />
      )}
