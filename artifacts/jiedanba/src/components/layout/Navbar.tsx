...
export function Navbar() {
  ...
  const levelLabel = profile.level === "A" ? "专家" : profile.level === "B" ? "进阶" : "新手";
    ...
    <nav className="fixed top-0 w-full z-50 glass-panel border-b border-border/50">
      <div className="max-w-[1920px] mx-auto px-6 lg:px-10 h-20 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          {/* User chip — synced to ProfileContext */}
          <Link href="/profile"
            className="flex items-center gap-3 pl-2 p-1 pr-4 rounded-full border border-border/50 hover:border-primary/30 hover:bg-primary/5 transition-all cursor-pointer">
            <div className="w-9 h-9 rounded-full bg-secondary/10 flex items-center justify-center text-secondary overflow-hidden shrink-0">
              {profile.avatar ? (
                <img src={profile.avatar} alt={profile.name} className="w-full h-full object-cover" />
              ) : (
                <span className="text-sm font-black text-primary">{profile.name?.[0] ?? "新"}</span>
              )}
            </div>
