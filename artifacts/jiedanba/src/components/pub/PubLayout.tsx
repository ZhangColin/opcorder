...
export function PubLayout({ children, title, backHref, backLabel, actions }: PubLayoutProps) {
  ...
  const { nickname, avatarChar } = useCurrentUser();
  ...
  return (
    <div className="flex min-h-screen bg-[#f9f9fc] text-[#1a1c1e] overflow-x-hidden">
      ...
      <main className="flex-1 lg:ml-64 min-h-screen min-w-0 overflow-x-hidden">
        <header className="fixed top-0 right-0 lg:left-64 left-0 z-40 bg-white/80 backdrop-blur-md shadow-sm flex items-center px-4 lg:px-8 py-3 gap-3">
          <button
          ...
          {backHref && (
            <button
              ...
              className="flex items-center gap-1.5 text-slate-500 hover:text-primary text-sm font-medium transition-colors"
            >
