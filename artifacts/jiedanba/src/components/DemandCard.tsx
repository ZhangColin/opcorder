...
export function DemandCard({ demand }: { demand: Demand }) {
  ...
  return (
    <div className="bg-card rounded-2xl p-6 border border-border shadow-sm hover:shadow-xl hover:border-primary/30 transition-all duration-300 group flex flex-col justify-between h-full relative overflow-hidden">
      {isUrgent && (
      ...
      <div className="pt-5 border-t border-border flex items-end justify-between mt-auto">
        <div>
          ...
          <div className="flex items-baseline gap-1">
            <span className="text-lg font-black text-secondary">¥{demand.budgetMin?.toLocaleString()}</span>
            <span className="text-xs text-muted-foreground font-bold">- {demand.budgetMax?.toLocaleString()}</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <Link href={`/demands/${demand.id}`} className="bg-primary/10 text-primary hover:bg-primary hover:text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 flex items-center group-hover:shadow-md">
            抢单
