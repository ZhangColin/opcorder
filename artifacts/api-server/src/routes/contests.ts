...
router.get("/admin/contests/questions", requireAdmin, async (req, res) => {
  try {
    const { catCategoryId } = req.query as Record<string, string>;
    ...
    return res.json({ data: rows, total: Number(total), page, pageSize });
  } catch (err) {
...
router.get("/admin/contests", requireAdmin, async (req, res) => {
  try {
    const { page, pageSize, offset } = paginate(req.query as Record<string, string | string[] | undefined>);
    const [{ total }] = await db.select({ total: count() }).from(contestsTable);
    ...
    const rows = await db
      .select({
        ...
        trackCount: sql<number>`(SELECT COUNT(*) FROM contest_tracks WHERE contest_id = ${contestsTable.id})`,
        registrationCount: sql<number>`(SELECT COUNT(*) FROM contest_registrations WHERE contest_id = ${contestsTable.id})`,
      })
      .from(contestsTable)
    ...
    return res.json({
      data: rows.map(r => ({ ...r, trackCount: Number(r.trackCount), registrationCount: Number(r.registrationCount) })),
...
/* ─── Admin: Registrations & Grading (报名与评级) ────────────────── */
...
router.post("/admin/contests/registrations/:id/grade-assignment", requireAdmin, async (req, res) => {
  try {
    ...
    // If passed, upsert opc_track_certs
    if (passed) {
    ...
    // TODO: SMS placeholder — sendContestGradeResult(phone, contestTitle, grade)
    // TODO: Email placeholder — sendContestGradeEmail(email, nickname, contestTitle, grade)
...
router.post("/contests/:id/tracks/:trackId/register", requireAuth, async (req, res) => {
  try {
    ...
    const [existing] = await db.select({ id: contestRegistrationsTable.id })
      ...
      .limit(1);
    if (existing) return res.status(400).json({ error: "您已报名该赛道" });
    ...
    const [reg] = await db.insert(contestRegistrationsTable).values({
      contestId,
      trackId,
      userId,
      status: "registered",
...
router.put("/contests/registrations/:id/assignment", requireAuth, async (req, res) => {
  try {
    ...
    // Check benefit_at and deadline
    const [contest] = await db.select({ benefitAt: contestsTable.benefitAt })
      .from(contestsTable).where(eq(contestsTable.id, reg.contestId)).limit(1);
    const [track] = await db.select({
      testGrade: sql<string>`''`, // placeholder — will use reg.testGrade
      aDurationHours: contestTracksTable.aDurationHours,
      bDurationHours: contestTracksTable.bDurationHours,
      cDurationHours: contestTracksTable.cDurationHours,
    }).from(contestTracksTable).where(eq(contestTracksTable.id, reg.trackId)).limit(1);
