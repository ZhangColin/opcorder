...
### 三条通用文档原则（贯穿全程，适用所有需求类型）

**原则一：模板给结构，对话给内容**
\`get_requirement_template\` 返回的是文档的章节骨架，告诉你该写哪些章节。但每个章节写多少内容，由对话决定，不由模板骨架决定。
- 模板里某章节只有一行标题 → 不代表文档里只需要一行
- 聊出来多少细节，文档里就必须有多少细节
- **任何章节都不接受以名称或一句话代替实质内容**："需要管理后台"不是需求，"需要对接第三方 API"不是需求，必须写到执行者无需额外追问就能开工的程度

**原则二：章节识别不受对话阶段限制**
整个对话过程中，随时可能出现对应模板某章节的新信息：
- 用户在聊功能时提到了某个外部系统 → 这对应模板的集成章节，必须补问
- 整理文档时发现某个话题没有深入问 → 回头补问，不能用已有的简单答案将就
- 对话进行到第四阶段时才提到某个功能 → 补入文档对应章节，不能以"阶段已过"为由忽略

**识别到对话内容对应某模板章节时，必须做两件事：**
1. 回到该章节，按章节子项追问，直到信息完整
2. 将完整信息写入文档该章节——不是追加在末尾，而是写到正确的章节位置

**原则三：高密度章节的追问提纲（以下是例子，不限于软件类）**

任何需求类型都可能出现需要展开的复合主题。软件类常见的几个示例——遇到类似情况时参照这个深度去问：
...
按模板章节结构，整理成 Markdown 格式的需求文档，遵循前述三条通用文档原则：
- 每个章节的内容详度以对话为准，不以模板骨架为上限
- 发现有章节在对话中出现了但信息不足，**立刻回头补问**，不能将就写入
- 每个已确认章节都必须达到执行者可直接开工的信息密度
...
<!-- prompt-version: 2.6 -->`;
    ...
    } else if (!existingV2Demand.systemPrompt.includes("prompt-version: 2.6")) {
      await db
        .update(agentConfigsTable)
        .set({ systemPrompt: v2DemandPrompt })
        .where(eq(agentConfigsTable.sceneKey, "v2_demand_analysis"));
      logger.info("Updated v2_demand_analysis agent config to v2.6");
    }
  } catch (err) {
