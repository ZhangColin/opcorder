...
const REQUIREMENT_TEMPLATES: Record<string, {
  ...
  software: {
    ...
    sections: [
      ...
      { name: "运行环境与对接要求", guide: "这个软件需要在什么环境下运行？（比如公司电脑、手机、网页浏览器等）需要和哪些已有的系统或平台打通？具体的技术实现方案由 OPC 自主决定。", required: false },
      { name: "数据与安全", guide: "这个软件会用到什么类型的数据？（如客户信息、财务数据等）有没有保密要求或行业合规规定需要遵守？", required: false },
      { name: "界面与使用体验", guide: "对软件界面外观有没有要求或偏好？有没有喜欢的参考案例？是追求美观的界面，还是以功能可用为主？", required: false },
      { name: "验收标准", guide: "所有功能完成后，您如何判断软件达标？（如全部功能正常使用、日常操作流畅不卡顿等）", required: true },
      { name: "交付后的使用与维护", guide: "软件交付后放在公司自己的服务器还是第三方平台上？是否需要配套操作手册，或者后续的维护支持？", required: false },
    ],
  },
  marketing: {
    name: "营销",
    ...
    sections: [
      ...
      { name: "核心卖点与传播主题", guide: "产品/服务最打动客户的优势是什么？这次宣传想主打的核心主题或广告语是什么？", required: true },
      { name: "预期效果指标", guide: "希望这次营销活动带来什么具体效果？（比如视频播放量达到多少、新增多少粉丝、带来多少销售额、有多少人来咨询等）", required: false },
  ...
  content: {
    ...
    sections: [
      ...
      { name: "品牌规范", guide: "是否有公司品牌形象规范需要遵循？（如指定的 Logo、字体、品牌主色、视觉规范手册等）如有请提供相关文件。", required: false },
      { name: "交付格式", guide: "需要交付哪种格式的文件？（如PPT、PDF、视频文件等）是否需要保留能继续修改的原始文件？", required: true },
